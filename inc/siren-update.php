...
function the_headPattern(){
  ...
  if(is_single()){
    ...
    }elseif(akina_option('thumbnail_o') == true && akina_option('focus_img_0')){
      $full_image_url = $random_thumbnail;
    }else{
      $full_image_url = $full_image_url[0];
    }
...
function siren_get_useragent($ua){
  if(akina_option('open_useragent')){
    ...
    return '<span class="useragent-info">「 <img src="'. $imgurl.$browser[1] .'.png"> ● <img src="'. $imgurl.$os[1] .'.png"> 」</span>';
  }
  return false;
}
